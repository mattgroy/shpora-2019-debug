    ...
    public class CompressedImage
    {
        ...
        public void Save(string path)
        {
            using (var sw = new FileStream(path, FileMode.Create))
            {
                ...
                buffer = BitConverter.GetBytes(this.Width);
                sw.Write(buffer, 0, buffer.Length);

                buffer = BitConverter.GetBytes(this.Height);
                sw.Write(buffer, 0, buffer.Length);

                buffer = BitConverter.GetBytes(this.Quality);
                sw.Write(buffer, 0, buffer.Length);

                buffer = BitConverter.GetBytes(this.DecodeTable.Count);
                sw.Write(buffer, 0, buffer.Length);

                foreach (var kvp in this.DecodeTable)
                {
                ...
                buffer = BitConverter.GetBytes(this.BitsCount);
                sw.Write(buffer, 0, buffer.Length);

                buffer = BitConverter.GetBytes(this.CompressedBytes.Length);
                sw.Write(buffer, 0, buffer.Length);

                sw.Write(this.CompressedBytes, 0, this.CompressedBytes.Length);
            }
        }
